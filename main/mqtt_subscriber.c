...
static const char *sub_topic = "/smarthome/devices";
...
extern const int MESSAGE_ARRIVE_BIT;
char data_rx[100];
extern json_gen_test_result_t result;
extern data_sensor_t data_sensor;
extern int json_parse_data_sensor(char *json, data_sensor_t *out_data);
...
static void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data)
{
	...
	else if (ev == MG_EV_MQTT_MSG)
	{
		...
		sprintf(data_rx, "%.*s", (int)mm->data.len, mm->data.ptr);
		json_parse_data_sensor(data_rx, &data_sensor);
