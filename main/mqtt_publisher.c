...
#endif
static const char *pub_topic = "/device/led1";
static const char *will_topic = "WILL";
extern EventGroupHandle_t s_mqtt_event_group;
extern const int MESSAGE_TX_ARRIVE_BIT;
extern char data_tx[100];
...
void mqtt_publisher(void *pvParameters)
{
	...
	while (1)
	{
		EventBits_t bits = xEventGroupWaitBits(s_mqtt_event_group,
											   MESSAGE_TX_ARRIVE_BIT,
											   pdTRUE,
											   pdTRUE,
											   0);
		ESP_LOGD(pcTaskGetName(NULL), "bits=%" PRIu32, bits);
		if ((bits & MESSAGE_TX_ARRIVE_BIT) != 0)
		{
			struct mg_str data = mg_str(data_tx);
