...
static const char *s_listen_on = "mqtt://0.0.0.0:8000";
const char *s_url = "ws://broker.hivemq.com:8000/mqtt";
const char *s_topic = "/smarthome/devices";
...
static void fn_broker(struct mg_connection *c, int ev, void *ev_data, void *fn_data)
{
	if (ev == MG_EV_MQTT_CMD)
	{
		...
		case MQTT_CMD_PUBLISH:
		{
			...
			for (struct sub *sub = s_subs; sub != NULL; sub = sub->next)
			{
				if (_mg_strcmp(mm->topic, sub->topic) != 0)
					continue;
...
void mqtt_server(void *pvParameters)
{
	...
	mg_mgr_init(&mgr);
	mg_mqtt_listen(&mgr, s_listen_on, fn_broker, NULL); // Create MQTT listener
